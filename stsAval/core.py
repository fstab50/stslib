...
class StsCore():
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            role_file (str): optional name of a json structured file located stsaval config
            directory in users home directory. File contains information about
            roles for which you which to generate temporary credentials

            output_file (str): optional non-default filename

            profile_name (str): user profile configured in local awscli config
            with permissions to assume roles in target aws accounts

            debug (bool):  optional debug flag (DEFAULT = False)
        """
        # validate provided kwargs
        keywords = ('role_file', 'output_file', 'profile_name', 'log_mode', 'debug')
        if self.filter_args(kwargs, *keywords):
            pre_transform = kwargs.get('role_file', None)
            post_transform = kwargs.get('output_file', defaults['output_file'])
        ...
        self.profiles = self.parse_profiles(pre_name=pre_transform, post_name=post_transform)
    ...
    def parse_profiles(self, pre_name=None, post_name=None):
        """
        Summary:
            creates list of account profiles from local configuration file

        Args:
            :: pre_name: input file containing iam role credentials in non-default
                         location or fname
            :: post_name: json file containing role profiles for which stsaval
                  will generate temporary credentials.  This file is generated by
                  stsAval and will be located in the ~/.stsaval directory. Format:
        ...
        Returns:
            profile_dict: list of aws account profile role names, role arns
            TYPE: dictionary
        """
        # pre_name:  input file - to be parsed
        # profile_file:  output file - after parsing awscli
        profile_file = self.config_dir + '/' + str(post_name)

        try:
            if self.refactor(input_file=pre_name, output_file=profile_file, force_rewrite=False):
                with open(profile_file) as f1:
                    profile_dict = json.load(f1)
            else:
                raise OSError(
                    '%s: Unknown problem parsing local awscli credentials file: %s' %
